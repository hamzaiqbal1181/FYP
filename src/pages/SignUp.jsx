...
// --- NEW: Import 'db' and Firestore functions ---
...
const SignUp = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      // --- NEW: Store extra details in Firestore Database ---
      // We use the user.uid as the document ID so it links easily to Auth
      await setDoc(doc(db, "users", user.uid), {
        firstName: firstName,
        lastName: lastName,
        contactInfo: contactInfo,
        email: email, // Optional: storing email in DB makes it easier to search later
        createdAt: new Date(),
      });
      // ----------------------------------------------------

      // Keep this if you want immediate access in the Navbar without fetching from DB
      localStorage.setItem("userFirstName", firstName);
